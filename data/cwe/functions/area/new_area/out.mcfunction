...
scoreboard players operation @e[type=marker,tag=cwe.new_area,limit=1] cwe.area.type = #type.out cwe.ids

execute as @e[type=marker,tag=cwe.new_area,limit=1] run function cwe:area/new_area/summon

scoreboard players operation @s cwe.working.out = created_area_id cwe.temp
