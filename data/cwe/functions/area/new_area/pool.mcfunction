...
scoreboard players operation @e[type=marker,tag=cwe.new_area,limit=1] cwe.area.type = #type.pool cwe.ids

execute as @e[type=marker,tag=cwe.new_area,limit=1] run function cwe:area/new_area/summon

scoreboard players operation @s cwe.working.pool = created_area_id cwe.temp
