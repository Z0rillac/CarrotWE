...
execute if score done cwelib.temp matches 1 run tellraw @s ["",{"text":"Filled ","color":"gold"},{"score":{"name":"outSize","objective":"cwelib.areas"},"color":"gold"},{"text":" blocks. ","color":"gold"},{"text":"[Select another block]","color":"green","clickEvent":{"action":"suggest_command","value":"/execute store success score @s cwe.fill run setblock 29999999 4 15360000 "}}]
