...
execute as @a[tag=cwe.gui.user,sort=nearest,limit=1] run function cwe:gui/predict
...
execute store result score roty cwe.temp run data get entity @a[tag=cwe.gui.user,sort=nearest,limit=1] Rotation[1]
execute unless score roty cwe.temp matches 80..90 run scoreboard players set @a[tag=cwe.gui.user,sort=nearest,limit=1] cwe.gui.minecart 3
