...
scoreboard players operation search cwe.sid = @e[tag=cwe.gui.user,sort=nearest,limit=1] cwe.working.in
execute as @e[tag=cwe.area,predicate=cwe:linked_to_head] run function cwe:area/remove
scoreboard players set @e[tag=cwe.gui.user,sort=nearest,limit=1] cwe.working.in 0
